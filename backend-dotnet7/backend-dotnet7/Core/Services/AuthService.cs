...
public class AuthService : IAuthService
{
    ...
    public async Task<GeneralServiceResponseDto> SeedRolesAsync()
    {
        ...
        bool isAdminRoleExists = await _roleManager.RoleExistsAsync(StaticUserRoles.ADMN);
        ...
        await _roleManager.CreateAsync(new IdentityRole(StaticUserRoles.ADMN));
    ...
    public async Task<GeneralServiceResponseDto> UpdateRoleAsync(ClaimsPrincipal User, UpdateRoleDto updateRoleDto)
    {
        ...
        if (User.IsInRole(StaticUserRoles.ADMN))
        {
            // User is admin
            if (updateRoleDto.NewRole == RoleType.USER || updateRoleDto.NewRole == RoleType.MANAGER)
            {
                ...
                if (userRoles.Any(q => q.Equals(StaticUserRoles.OWNER) || q.Equals(StaticUserRoles.ADMN)))
                {
                    return new GeneralServiceResponseDto()
                    {
                        IsSucceed = false,
